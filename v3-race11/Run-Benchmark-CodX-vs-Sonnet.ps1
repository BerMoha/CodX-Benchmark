...
# Both scored by same cosine metric (BAAI/bge-small-en-v1.5)
...
Write-Log " Scorer: BAAI/bge-small-en-v1.5 (local)"
Write-Log "=============================================="
...
$csvHeader = "test_id,domain,level,prompt_summary,codx_score,codx_time_s,codx_cost_usd,codx_winner_model,sonnet_score,sonnet_time_s,sonnet_cost_usd,winner,date"
...
foreach ($q in $toRun) {
    ...
    try {
        ...
        $codxWinnerModel = [string]$codxResp.tournament_winner
        $codxOk = $true
        $csRound = [math]::Round($codxScore, 4)
        Write-Log "  CodX   : ${codxTime}s | score: $csRound | cost: `$$codxCost | model: $codxWinnerModel" "OK"
    } catch {
        $codxTime = [math]::Round(((Get-Date) - $sw).TotalSeconds, 2)
    ...
    $row = "$($q.Id),$($q.Domain),$($q.Level),$summary,$cs,$codxTime,$codxCost,$codxWinnerModel,$ss,$sonnetTime,$sonnetCost,$winner,$BenchmarkDate"
